function result = simulate(controller, x_desired,y_desired,z_desired)
    ...
    for t = times
        ...
        %STORE SETTLING TIME
        if settlingtime==0
            if x(1)>0.98*x_desired && x(1)<1.02*x_desired...
                    && x(2)>0.98*y_desired && x(2)<1.02*y_desired...
                    && x(3)>0.98*z_desired && x(3)<1.02*z_desired

                settlingtime=times(count);
            end
        end           
    ...
    result = struct('x',xout,'theta',thetaout,'vel',xdotout,'angvel'...
        ...
        ,'integralerrors',integralerrors,'settlingtime',settlingtime);  
